    ...
    public static class NotificationService
    {
        ...
        public static void CreateOSATConfirmNotificationsWhileLotChanged(string osatID, Lot lot, string message)
        {
            ...
            foreach (OSATUser user in UserService.GetOSATUsersByOSATID(osatID))
            {
                ...
                notificationGateway.AddNew(newNotification);
                string str = emailGateway.GetEmailByID("OSATNotification").Body.Replace("InsertFullNameHere", user.SupUserName).Replace("insertLotNoHere", lot.LotNO).Replace("InsertUrlHere", string.Format("https://lhd.unisoc.com/Lots/Details?LotID={0}", lot.LotID)).Replace("InsertTimeHere", DateTime.Now.ToString("yyyy-MM-dd hh:MM:ss"));
        ...
        public static void CreateOSATEQANotificationsWhileLotArrived(string osatID, Lot lot, string message)
        {
            ...
            foreach (OSATUser user in UserService.GetOSATUsersByOSATID(osatID))
            {
                ...
                notificationGateway.AddNew(newNotification);
                string str = emailGateway.GetEmailByID("OSATEQANotification").Body.Replace("InsertFullNameHere", user.SupUserName).Replace("insertLotNoHere", lot.LotNO).Replace("InsertUrlHere", string.Format("https://lhd.unisoc.com/Lots/Details?LotID={0}", lot.LotID)).Replace("InsertTimeHere", DateTime.Now.ToString("yyyy-MM-dd hh:MM:ss"));
